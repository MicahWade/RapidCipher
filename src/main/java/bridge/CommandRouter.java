    ...
    * This class is now thread-safe and returns responses directly.
    */
    public class CommandRouter {
        ...
        * This method is called from the ClientHandler thread.
        */
        ...
        /**
        * "Direct Mode" Handler: Authenticates, decrypts, and returns logins in one go.
        ...
        * This uses Platform.runLater and blocks the calling thread (ClientHandler)
        * until the GUI prompt is finished.
        */
        private JsonObject requestUnlock() {
            ...
            try {
                latch.await(); // Wait for the GUI thread to finish
        ...
        */
        private synchronized JsonObject getLogins(SecretKey key) {
            // Ensure database is initialized
            if (database == null) {
                try {
                    System.out.println("CommandRouter: Database is null, initializing...");
            ...
            try {
                ...
                if (rs == null) {
                    throw new SQLException("Failed to execute search, searchLogins() returned null.");
                }
                while (rs.next()) {
                    JsonObject login = new JsonObject();
                    long id = rs.getLong("id");
                    login.addProperty("id", id);
                    ...
                    try {
                        ...
                        login.addProperty("name", "!!DECRYPT_ERROR!!");
                        login.addProperty("username", "!!DECRYPT_ERROR!!");
                        // Add other fields as errors if needed
                    }
                ...
                System.err.println("CommandRouter: SQL Error: " + e.getMessage());
                // Try to close and reconnect on SQL exception
                try {
                    System.err.println("CommandRouter: Attempting to reconnect database...");
                    if (database != null) database.closeConnection();
                    this.dbConfig = ConfigManager.loadConfig();
                    this.database = new Database(dbConfig);
                    System.err.println("CommandRouter: Reconnect successful.");
                    // We don't retry here, we just report the initial error.
                    // The connection will be ready for the *next* request.
                } catch (Exception ex) {
                    System.err.println("CommandRouter: Failed to reconnect database: " + ex.getMessage());
                    return createErrorResponse("Failed to reconnect to database: " + ex.getMessage());
                }
                return createErrorResponse("Failed to retrieve logins: " + e.getMessage());
                
            } catch (Exception e) {
                System.err.println("CommandRouter: General Error: " + e.getMessage());
